...
const statCards = (m: NetworkMetrics) => [
  { label: 'People', value: m.node_count, color: 'text-indigo-400', bg: 'bg-indigo-500/10' },
  { label: 'Communities', value: m.communities, color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
  { label: 'Density', value: m.density.toFixed(3), color: 'text-amber-400', bg: 'bg-amber-500/10' },
  { label: 'Components', value: m.components, color: 'text-rose-400', bg: 'bg-rose-500/10' },
]
    ...
    <aside className="w-56 shrink-0 bg-gray-950 border-r border-white/5 p-4 flex flex-col gap-6 overflow-y-auto">
      <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {statCards(metrics).map(({ label, value, color, bg }) => (
